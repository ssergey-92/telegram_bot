"""Module for handling low_price command."""

from .common import msg_type_city_name
from config_data.config import LOW_PRICE_COMMAND_DATA
...
initialization_data = {
    "command":  LOW_PRICE_COMMAND_DATA["shortcut"],
    "min_price": 1,
    "max_price": 1000000,
    "sort": "PRICE_LOW_TO_HIGH",
}
msg_selected_low_price = "Selected {command_shortcut}".format(
    command_shortcut=LOW_PRICE_COMMAND_DATA["shortcut"]
)


def handle_low_price_command(chat_id: int, user_id: int) -> None:
    """Handle low_price command from user.
    ...
    bot.send_message(chat_id, msg_selected_low_price)
    ...
    history_id = HistoryCRUD.create_entry(
        user_id, LOW_PRICE_COMMAND_DATA["shortcut"],
    )
